...
// Unified Analysis Modal - combines single and batch analysis
class UnifiedAnalysisModal extends obsidian.Modal {
    ...
    openFileBrowser(selectedFileDisplay) {
        ...
        new obsidian.Notice('Click on a file in the explorer to select it');
    }

    // Open Obsidian's file browser for batch file selection
    openBatchFileBrowser(selectedFilesList) {
        const fileExplorer = this.app.workspace.getLeavesOfType('file-explorer')[0];
        if (!fileExplorer) {
            new obsidian.Notice('File explorer not found');
            return;
        ...
        // Show a notice to guide the user
        new obsidian.Notice('Click on files in the explorer to select them. Click "Done" when finished.');
    ...
    async performAnalysis(isSingleMode, useCurrentFile, exportFormat) {
        try {
            if (isSingleMode) {
                ...
                // Batch analysis
                if (this.selectedFiles.length === 0) {
                    new obsidian.Notice('No files selected for batch analysis');
                    return;
                }
...
module.exports = { UnifiedAnalysisModal };
